...
class SummarizeAgent:
    ...
    def execute(self, state: ResearchState) -> ResearchState:
        ...
        logger.info(
            f"Summarize agent executing with {len(search_results)} search results"
        )
        ...
        if not search_results:
            ...
            errors.append("Summarize agent error: No search results to summarize")
            return {**state, "errors": errors}

        summary_chain = self.chain_builder.build(
            prompt_template=PromptTemplates.Summarize_prompt(),
            input_vars=["results"],
            model=Summary,
        )
        ...
        try:
            ...
            logger.exception(f"Summarize agent failed: {e}")
            errors = state.get("errors", [])
