...
class FactCheckAgent:
    def __init__(
        self, llm, confidence_threshold: float, max_retries: int, add_max_results: int
    ):
        self.chain_builder = ChainBuilder(llm)
        self.confidence_threshold = confidence_threshold
        self.max_retries = max_retries
        self.add_max_results = add_max_results

    def execute(self, state: ResearchState) -> ResearchState:
        summary = state.get("summarized_content")
        search_results = state.get("search_results", [])

        # Increment fact check attempt counter
        ...
        if not summary or not search_results:
            return {**state, "errors": ["Fact check agent error: Missing data"]}

        fact_check_chain = self.chain_builder.build(
            prompt_template=PromptTemplates.fact_check_prompt(),
        ...
        try:
            ...
            logger.info(
                f"Fact check agent completed review. Accurate: {fact_check_results.is_accurate}"
            )
            logger.info(f"Confidence score: {fact_check_results.confidence_score}")
            ...
            if confidence_score < self.confidence_threshold:
                if retry_count < self.max_retries:
                    ...
                    if state["search_retries"] < self.max_retries:
                        logger.info(f"Retrying search #{state['search_retries']}")

                        state["max_results"] = max_results + self.add_max_results

                        logger.info(f"Increasing max_results to {state['max_results']}")

            state["fact_checked_results"] = fact_check_results
            return state

        except Exception as e:
            logger.exception(f"Fact check agent failed: {e}")

            return ErrorHandler.add_error(
                state,
                f"Fact check agent error: {str(e)}",
            )
